...
- **次要用户**：想要理解 Transformer 内部行为的学生与初学者
- **非目标用户**：生产环境部署工程师（本工具仅用于离线诊断分析）

### 2.2 典型使用场景

1. **研究者在优化模型前，先用 UniVis 跑一遍推理**，看看哪些层冗余度高，为后续剪枝/蒸馏提供数据依据
2. **学生在课程项目或自学时**，想直观理解"大模型内部每一层在干什么"
3. **技术展示与评审**：观众看到热力图从红变灰的动态过程，直观理解"后面的层计算冗余"

### 2.3 用户画像（真实）

维护者画像：有 React + Next.js 前端经验，有 Python 和 PyTorch 基础，正在积累系统级工程经验。目标是做一个开发难度可控、演示效果好、有真实分析价值的工具。
...
**可行性**：中等。需要在 2-3 个模型上做实验，每个模型跑 5-10 个 benchmark，工作量大但方法论清晰。后续可作为独立的研究方向继续推进。

### 6.3 建议策略

1. **工具阶段**：专注把工具做好，不追求论文
2. **工具稳定后**：如果跑出了有趣的冗余模式数据，再讨论是否有论文空间
